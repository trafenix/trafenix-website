...
const proofPoints = [
  { value: "13", label: "traffic analysis services" },
...
export default function Home() {
  return <><SiteHeader/><main>
    <section className="hero"><div className="hero-grid"/><div className="shell hero-inner">
      <div className="hero-copy reveal"><span className="eyebrow">Traffic video intelligence · UK & US</span><h1>Your Footage.<br/>Our Analysis.<br/><em>Actionable Insights.</em></h1><p>Send us recorded footage. Trafenix transforms it into structured, quality-checked traffic data using AI-assisted processing and expert manual verification.</p><div className="hero-actions"><a className="button" href="/contact">Get a project quote <ArrowRight size={18}/></a><a className="button secondary" href="/services">Explore 13 services</a></div><div className="hero-trust"><span><ShieldCheck size={17}/> Project-specific QA</span><span><Clock3 size={17}/> 24-hour standard delivery</span></div></div>
    ...
    <section className="section" id="services"><div className="shell"><div className="section-head"><div><span className="eyebrow dark">What we measure</span><h2>Thirteen services.<br/>One clear data partner.</h2></div><p>Choose one specialist service or combine several into a project-focused analysis.</p></div><div className="service-grid">{services.slice(0,6).map(service=><ServiceCard key={service.slug} service={service}/>)}</div><div className="center"><a className="button secondary dark-button" href="/services">View all 13 services <ArrowRight size={18}/></a></div></div></section>
